...
vector<int> kmp(string img, vector<int> pi_vec){
    ...
    do{
        if (c == img[img_ind]){               // если два одинаковых символа двигаемся вперед и по образу и по строке
            ...
            img_ind++;
            if (img_ind == img_s){ // если дошли до конца образа, значит нашли вхождение
        ...
        else if (img_ind == 0){ // если нет совпадения, и сравнение происходит не с самым первым символом образа
            text_ind++;
            if(! (cin >> c))correct = false;
        }
        else
